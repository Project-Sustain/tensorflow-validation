...
def validate_model(id, path, type, database, collection, label_field, validation_metric, feature_fields, gis_joins):
    client = MongoClient(URI)
    database = client[database]
    collection = database[collection]

    documents = collection.find({'COUNTY_GISJOIN': 'G4802970'}, {'_id': 0, feature_fields[0]: 1, label_field: 1})
    features = []
    labels = []
    num_processed = 0
